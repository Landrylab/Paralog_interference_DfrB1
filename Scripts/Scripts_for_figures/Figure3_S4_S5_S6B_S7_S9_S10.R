...
#### New code: Check correlations between replicates ####

table(dms_data_check_TAG$Background)

# Convert to wide
dms_data_check_TAG_wide <- dms_data_check_TAG %>% rowwise() %>%
  mutate(Sample_ID = str_c('Run_', toString(Run), '.Replicate_', toString(Replicate), sep = '')) %>%
  select(-Ranked_sel_coeff, -Run, -Replicate) %>%
  pivot_wider(names_from = Sample_ID, values_from = Selection_coefficient, names_prefix = 'sel_coeff_')

# Load data
data_read_counts <- read_delim('Data/DMS_bulk_competition_experiments/DMS_read_count_Duplicated_Singleton.tsv', delim = '\t')

data_read_counts_t0 <- data_read_counts %>% 
  separate(col = ID, into = c('tmp1', 'timepoint_2', 'tmp2', 'tmp3'), sep = '_', remove = F) %>%
  filter(Timepoint == 0, timepoint_2 == '0', read_counts >= 100) %>% select(-tmp1, -tmp2, -tmp3)

data_read_counts_t10 <- data_read_counts %>% 
  separate(col = ID, into = c('tmp1', 'timepoint_2', 'tmp2', 'tmp3'), sep = '_', remove = F) %>%
  filter(Timepoint == 10, timepoint_2 == '10') %>% select(-tmp1, -tmp2, -tmp3)

## Calculate read proportions at t0 ##
wt_read_counts_t0 <- data_read_counts_t0 %>% 
  filter(WT_Codon == Codon, Timepoint == 0, Position == 10) %>% # Avoid repeating WT at each position
  arrange(Sample, Run, Replicate, Position)

# Get the total number of reads for each run and replicate
total_read_counts_t0 <- data_read_counts_t0 %>% ungroup() %>%
  filter(!(and(WT_Codon == Codon, Position > 10))) %>% ## Remove the repeated counts of WT
  group_by(Sample, Run, Replicate, Lane) %>%
  summarise(total_reads = sum(read_counts))
 
## Calculate the read proportions at t0
data_read_proportions_t0 <- left_join(x = data_read_counts_t0 %>% 
                                        filter(!(and(WT_Codon == Codon, Position > 10))), ## Remove repeated WT 
                                      y = total_read_counts_t0, 
                                      by = c('Sample' = 'Sample', 'Run' = 'Run', 
                                             'Replicate' = 'Replicate', 'Lane' = 'Lane')
) %>%
  mutate(read_proportion = (read_counts + 1) / total_reads)

## The sum of all read proportions should be 1
test_proportions_t0 <- data_read_proportions_t0 %>% ungroup() %>%
  group_by(Sample, Run, Replicate, Lane) %>%
  summarise(total_prop = sum(read_proportion))

## Calculate read proportions at t10 ##
wt_read_counts_t10 <- data_read_counts_t10 %>% 
  filter(WT_Codon == Codon, Timepoint == 10, Position == 10) %>% # Avoid repeating WT at each position
  arrange(Sample, Run, Replicate, Position)

# Get the total number of reads for each run and replicate
total_read_counts_t10 <- data_read_counts_t10 %>% ungroup() %>%
  filter(!(and(WT_Codon == Codon, Position > 10))) %>% ## Remove the repeated counts of WT
  group_by(Sample, Run, Replicate, Lane) %>%
  summarise(total_reads = sum(read_counts))

## Calculate the read proportions at t10
data_read_proportions_t10 <- left_join(x = data_read_counts_t10 %>% 
                                        filter(!(and(WT_Codon == Codon, Position > 10))), ## Remove repeated WT 
                                      y = total_read_counts_t10, 
                                      by = c('Sample' = 'Sample', 'Run' = 'Run', 
                                             'Replicate' = 'Replicate', 'Lane' = 'Lane')
) %>%
  mutate(read_proportion = (read_counts + 1) / total_reads)
...
## The sum of all read proportions should be 1
test_proportions_t10 <- data_read_proportions_t10 %>% ungroup() %>%
  group_by(Sample, Run, Replicate, Lane) %>%
  summarise(total_prop = sum(read_proportion))


## Calculate the correlations between read proportions
data_read_proportions_t0_wide <- data_read_proportions_t0 %>% rowwise() %>%
  mutate(new_ID = str_c('Sample_', toString(Sample), '.Run_', toString(Run), 
                        '.Rep_', toString(Replicate), '.Lane_', toString(Lane), sep = '')) %>%
  select(new_ID, Position, WT_Codon, Codon, WT_Residue, Residue, read_proportion) %>%
  pivot_wider(names_from = new_ID, values_from = read_proportion, names_prefix = 'read_prop_')

## Confirm that the two lanes are always identical
test1 <- data_read_proportions_t0_wide %>% 
  filter(!(is.na(read_prop_Sample_Duplicated.Run_1.Rep_1.Lane_1)), !(is.na(read_prop_Sample_Duplicated.Run_1.Rep_1.Lane_2)))

all(test1$read_prop_Sample_Duplicated.Run_1.Rep_1.Lane_1 == test1$read_prop_Sample_Duplicated.Run_1.Rep_1.Lane_2)
# TRUE

test2 <- data_read_proportions_t0_wide %>% 
  filter(!(is.na(read_prop_Sample_Singleton.Run_1.Rep_1.Lane_1)), !(is.na(read_prop_Sample_Singleton.Run_1.Rep_1.Lane_2)))

all(test2$read_prop_Sample_Singleton.Run_1.Rep_1.Lane_1 == test2$read_prop_Sample_Singleton.Run_1.Rep_1.Lane_2)
# TRUE

table(dms_data_check_TAG$Background, dms_data_check_TAG$Run)

data_read_proportions_t0_wide_new <- data_read_proportions_t0 %>% rowwise() %>%
  filter(!(and(Sample == 'Duplicated', Run == 2)), ## Remove run 2 for duplicated
         Lane == 1) %>% # Keep only lane 1
  mutate(new_ID = str_c('Sample_', toString(Sample), '.Run_', toString(Run), 
                        '.Rep_', toString(Replicate), sep = '')) %>%
  select(new_ID, Position, WT_Codon, Codon, WT_Residue, Residue, read_proportion) %>%
  pivot_wider(names_from = new_ID, values_from = read_proportion, names_prefix = 'read_prop_')


data_read_proportions_t0_wide %<>% 
  mutate(log2_read_prop_Sample_Duplicated.Run_1.Rep_1.Lane_1 = log2(read_prop_Sample_Duplicated.Run_1.Rep_1.Lane_1), 
         log2_read_prop_Sample_Singleton.Run_1.Rep_1.Lane_1 = log2(read_prop_Sample_Singleton.Run_1.Rep_1.Lane_1),
         log2_read_prop_Sample_Singleton.Run_2.Rep_1.Lane_1 = log2(read_prop_Sample_Singleton.Run_2.Rep_1.Lane_1)
         )
...
dms_data_codon_wide <- dms_data_codon %>% ungroup() %>%
  ...
  separate(col = Mutation, into = c('WT_Residue', 'Position', 'Residue'), sep = c(1, -1), remove = F) %>%
...
## Add the corresponding WT read counts to the dfs of read proportions at t0 and t10
data_all_proportions_t0 <- left_join(x = data_read_proportions_t0 %>%
                                       select(Sample, Run, Timepoint, Replicate, Lane,
                                              WT_Codon, Position, Codon, WT_Residue, Residue,
                                              read_counts, total_reads, read_proportion), 
                                     y = wt_read_counts_t0 %>%
                                       mutate(read_counts_wt = read_counts) %>%
                                       select(Sample, Run, Timepoint, Replicate, Lane,
                                              read_counts_wt), 
                                     by = c('Sample' = 'Sample', 'Run' = 'Run', 'Timepoint' = 'Timepoint', 
                                            'Replicate' = 'Replicate', 'Lane' = 'Lane')
                                     ) %>%
  mutate(read_proportion_wt = read_counts_wt / total_reads) %>%
  mutate(ratio_t0 = read_proportion / read_proportion_wt)

data_all_proportions_t10 <- left_join(x = data_read_proportions_t10 %>%
                                        select(Sample, Run, Timepoint, Replicate, Lane,
                                               WT_Codon, Position, Codon, WT_Residue, Residue,
                                               read_counts, total_reads, read_proportion), 
                                      y = wt_read_counts_t10 %>%
                                        mutate(read_counts_wt = read_counts) %>%
                                        select(Sample, Run, Timepoint, Replicate, Lane,
                                               read_counts_wt), 
                                      by = c('Sample' = 'Sample', 'Run' = 'Run', 'Timepoint' = 'Timepoint', 
  ...
  mutate(read_proportion_wt = read_counts_wt / total_reads) %>%
  mutate(ratio_t10 = read_proportion / read_proportion_wt)

## Merge the two dataframes
data_recalc_sel_coeff <- left_join(x = data_all_proportions_t10 %>%
                                     filter(Lane == 1) %>% ## We showed above that the two lanes are identical 
                                     mutate(read_counts_t10 = read_counts,
                                            read_proportion_t10 = read_proportion,
                                            read_proportion_wt_t10 = read_proportion_wt, 
                                            
                                            total_reads_t10 = total_reads
                                            
                                            ) %>%
                                     select(Sample, Run, Replicate,
                                            WT_Codon, Position, Codon, WT_Residue, Residue,
                                            read_proportion_wt_t10, total_reads_t10,
                                            read_counts_t10, read_proportion_t10, ratio_t10),
                                   y = data_all_proportions_t0 %>%
                                     filter(Lane == 1) %>%
                                     mutate(read_counts_t0 = read_counts,
                                            read_proportion_t0 = read_proportion,
                                            read_proportion_wt_t0 = read_proportion_wt,
                                            
                                            total_reads_t0 = total_reads
                                            ) %>%
                                     select(Sample, Run,
                                            WT_Codon, Position, Codon, WT_Residue, Residue,
                                            total_reads_t0, ratio_t0, 
                                            read_proportion_t0, read_counts_t0), 
                                   by = c('Sample' = 'Sample', 'Run' = 'Run', 
                                          'WT_Codon' = 'WT_Codon', 'Position' = 'Position',
                                          'Codon' = 'Codon', 'WT_Residue' = 'WT_Residue',
                                          'Residue' = 'Residue'
                                          ))


data_recalc_sel_coeff %<>% 
  mutate(new_sel_coeff = log2(ratio_t10 / ratio_t0) / 10, 
         max_del_sel_coeff = log2( ( (1/total_reads_t10) /  read_proportion_wt_t10) / ratio_t0) / 10
         )

## Check if the recalculated selection coefficients are the same as Florian's
test_sel_coeff <- inner_join(x = dms_data_check_TAG %>%
                               mutate(Position = as.numeric(Position)), 
                             y = data_recalc_sel_coeff, 
                             by = c('Background' = 'Sample', 'Run' = 'Run', 
                                    'Replicate' = 'Replicate', 'WT_Codon' = 'WT_Codon', 
                                    'Position' = 'Position', 'Mutant_Codon' = 'Codon'
                                    
                                    )
                             )

## Because I added a pseudocount, the WT codons are off by a slight magnitude. Replace them with zero.
test_sel_coeff %<>% 
  mutate(new_sel_coeff = ifelse(WT_Codon == Mutant_Codon, 0, new_sel_coeff))
## Data is ready but needs annotations of interfering mutations that are obtained later

#### Identify interfering mutations ####
...
sem_stop_EMPTY <- sem_stop

## When comparing to the distribution of stop codons
stop_EMPTY_high <- mean_stop + cutoff_sd*sd_stop_EMPTY
stop_EMPTY_low <- mean_stop -  cutoff_sd*sd_stop_EMPTY

## When comparing to the mean of stop codons
# stop_EMPTY_high <- mean_stop + cutoff_sd*sem_stop_EMPTY
# stop_EMPTY_low <- mean_stop -  cutoff_sd*sem_stop_EMPTY
...
# 187 mutations within 2.5 s.e.m of the mean of stop codons (Singleton)

sum(dms_data_final_deleterious$neg_interference)
# 2 "interfering" (Singleton)
# 333 "interfering" (Duplicated)
...
## Save the selection coefficients for stop codons in the experiment with the duplication
stop_codons_dup <- stop_codons_wt

## Save data for stop codons
mean_stop_CDS <- mean_stop
sd_stop_CDS <- sd_stop
sem_stop_CDS <- sem_stop
...
## When comparing to the mean of stop codons
# stop_CDS_high <- mean_stop_CDS + cutoff_sd*sem_stop_CDS
# stop_CDS_low <- mean_stop_CDS - cutoff_sd*sem_stop_CDS
...
# 100 mutations within 2.5 s.e.m. of the mean of stop codons (Duplicated)

sum(dms_data_final_deleterious_CDS$neg_interference) 
# 63 interfering (Duplicated)
# 318 interfering (Duplicated)
...
## 63 mutations (mean - 2.5 sd)
## 351 mutations (mean - 2.5 s.e.m)

#### Check differences in effects between interfering mutations and stop codons ####

## Annotate the interfering mutations and the stop codons
dms_data_final_both_long_new <- dms_data_final_both_long %>% ungroup() %>%
  separate(col = Mutation, into = c('WT_Residue', 'Position', 'Residue'), 
           sep = c(1, -1), remove = F) %>%
  mutate(category = case_when(
    Mutation %in% neg_dom_check$Mutation ~ 'Interfering', 
    Residue == '*' ~ 'Stop', 
    TRUE ~ 'Other'
  )) %>%
  mutate(category = factor(category, levels = c('Interfering', 'Stop', 'Other')))

#### Figure S6B: The deleterious effects of interfering mutations are not rescued by the second copy ####

dms_data_final_both_diff <- dms_data_final_both_long_new %>% ungroup() %>%
  group_by(Mutation, WT_Residue, Position, Residue, category) %>%
  pivot_wider(names_from = Condition, values_from = Selection_coefficient, names_prefix = 's_') %>%
  mutate(diff_s = s_Duplicated - s_Singleton)

p_figS6B <- dms_data_final_both_diff %>%
  ggplot(aes(x = category, y = diff_s, colour = category)) +
  scale_colour_manual(values = c('#1a53ff', 'red', 'black')) +
  geom_jitter(width = 0.2) +
  geom_boxplot(width = 0.7, alpha = 0.4) +
  geom_violin(width = 0.7, scale = 'width', alpha = 0.4) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  stat_compare_means(comparisons = list(c('Interfering', 'Stop'),
                                         c('Interfering', 'Other'),
                                         c('Stop', 'Other')
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), 
        legend.position = 'none') +
  labs(y = expression(bold('\u0394s (Duplicated - Singleton)')))
p_figS6B
ggsave(p_figS6B, device = cairo_pdf, width = 12, height = 12, units = 'cm', dpi = 300, 
       filename = 'Figures/Supp_figures/FigS6B.pdf')
## Figure S6A was designed and added with Inkscape

#### Figure S7: Estimate the dynamic range of deleterious effects for each variant ####

test_sel_coeff_interfering <- test_sel_coeff %>% rowwise() %>%
  mutate(substitution_ID = str_c(WT_Residue, toString(Position), Residue, sep = '')) %>%
  ## Annotate interfering mutations and stop codons
  mutate(category = case_when(
    substitution_ID %in% neg_dom_check$Mutation ~ 'Interfering', 
    Residue == '*' ~ 'Stop', 
    TRUE ~ 'Other'
  )) %>%
  mutate(category = factor(category, levels = c('Interfering', 'Stop', 'Other')))

p_figS7 <- test_sel_coeff_interfering %>% rowwise() %>%
  mutate(Run = str_c('Run = ', toString(Run)),
         Replicate = str_c('Replicate = ', toString(Replicate))) %>%
  filter(Background == 'Duplicated') %>%
  ggplot(aes(x = max_del_sel_coeff, y = Selection_coefficient)) +
  facet_grid(Run ~ Replicate) +
  geom_point(aes(alpha = category, colour = category)) +
  scale_colour_manual(values = c('#1a53ff', 'red', 'black')) +
  scale_alpha_manual(values = c(0.7, 0.7, 0.1)) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed', col = 'red') +
  xlab('Selection coefficient (detection limit)') +
  ylab('Selection coefficient (observed)') +
  ggtitle('Duplicated samples, codon level')+
  theme(plot.title = element_text(hjust = 0.5), 
        legend.position = 'top', 
        legend.justification = 'center') +
  labs(alpha = '', colour = '')
p_figS7
ggsave(p_figS7, device = cairo_pdf, width = 27, height = 9, units = 'cm', dpi = 300, 
       filename = 'Figures/Supp_figures/FigS7.pdf')
...
dom_eff_candidates <- dms_data_final %>%
  ...
  filter(Selection_coefficient < (mean_stop-cutoff_sd*sd_stop_CDS), Mutant_Residue != '*')
  ## Filter by mean of selection coefficient minus 2.5 s.e.m
  # filter(Selection_coefficient < (mean_stop-cutoff_sd*sem_stop_CDS), Mutant_Residue != '*')
...
write.table(dom_eff_candidates, append = F, quote = F, sep = '\t', row.names = F, col.names = T, 
            ...
            # file = 'Supp_tables/TableS4_DominantNegativeCandidates_sem_stop.tsv')
...
dom_eff_candidates_regions <- df_volcano_plot_regions %>% 
  ...
  filter(Selection_coefficient < (mean_stop-2.5*sd_stop_CDS), p_adj <= 0.05)
  ## Filter based on the mean of stop codons - 2.5*(sd of stop codons)
  # filter(Selection_coefficient < (mean_stop-2.5*sem_stop_CDS), p_adj <= 0.05)
...
#### Figure S10: Deconvolute the relative enrichment within overlapping regions ####

## Dimerization interface ##
dom_eff_dim_int <- dom_eff_candidates_regions %>% filter(Site == 'Dimerization interface')

## Annotate positions from the dimerization interface that are also part of the active site 
test_positions_catalytic <- test_positions %>% filter(Site == 'NADPH binding')

test_positions_all_dim <- test_positions %>% filter(Site == 'Dimerization interface') %>%
  mutate(New_site = ifelse(Position %in% test_positions_catalytic$Position,
                           'NADPH binding', 'Only dimerization'))

total_missense_all_dim <- table(test_positions_all_dim$New_site)*19
# 76 (overlap with NADPH binding site), 171 (no overlap with catalytic site)

## Calculate expectations
expected_dom_eff_dim <- as.data.frame(round(total_missense_all_dim*(total_candidates / total_missense), 2))
# 3.65 (overlap with NADPH binding site), 8.22 (no overlap with NADPH binding site)

## Get observed counts and add a column for expected counts
dom_eff_dim_int_new <- left_join(x = dom_eff_dim_int %>%
                                   select(WT_Residue, Position, Mutant_Residue, Selection_coefficient), 
                                 y = test_positions_all_dim %>%
                                   select(Position, New_site), 
                                 by = c('Position' = 'Position')
)

dom_eff_dim_int_summary <- dom_eff_dim_int_new %>% ungroup() %>%
  group_by(New_site) %>%
  summarise(Obs_count = n())

data_deconv_dim <- left_join(x = dom_eff_dim_int_summary, 
                             y = expected_dom_eff_dim %>% mutate(New_site = Var1, Exp_count = Freq) %>%
                               select(New_site, Exp_count), 
                             by = c('New_site' = 'New_site')) %>%
  mutate(Rel_enrichment = round(Obs_count / Exp_count, 2))

p_deconv_dim <- data_deconv_dim %>% rowwise() %>%
  mutate(label_enrichment = str_c(toString(Rel_enrichment), '\n', 
                                  toString(Obs_count), '/', toString(Exp_count), 
                                  sep = ''), 
         New_site = factor(New_site, levels = c('Only dimerization', 'NADPH binding'))) %>%
  ggplot(aes(x = New_site, y = Rel_enrichment)) +
  geom_bar(stat = 'identity') +
  geom_hline(yintercept = 1, linetype = 'dashed') +
  geom_label(aes(label = label_enrichment, x = New_site, y = Rel_enrichment + 1),
             vjust = 0.5, size = 3.5
  ) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
        legend.position = 'none', 
  ...
  ylab('Relative enrichment\n(neg. int. mutations)') +
  ylim(0, max_rel_enrichment + rel_enrichment_margin) +
  ggtitle('Dimerization interface')
p_deconv_dim

## Tetramerization interface ##

dom_eff_tet_int <- dom_eff_candidates_regions %>% filter(Site == 'Tetramerization interface')

## Annotate positions from the dimerization interface that are also part of the active site 
test_positions_catalytic <- test_positions %>% filter(Site == 'NADPH binding')

test_positions_all_tet <- test_positions %>% filter(Site == 'Tetramerization interface') %>%
  mutate(New_site = ifelse(Position %in% test_positions_catalytic$Position,
                           'NADPH binding', 'Only tetramerization'))

total_missense_all_tet <- table(test_positions_all_tet$New_site)*19
# 76 (overlap with NADPH binding site), 133 (no overlap with NADPH binding site)

## Calculate expectations
expected_dom_eff_tet <- as.data.frame(round(total_missense_all_tet*(total_candidates / total_missense), 2))
# 3.65 (overlap with catalytic site), 6.39 (no overlap with catalytic site)

## Get observed counts and add a column for expected counts
dom_eff_tet_int_new <- left_join(x = dom_eff_tet_int %>%
                                   select(WT_Residue, Position, Mutant_Residue, Selection_coefficient), 
                                 y = test_positions_all_tet %>%
                                   select(Position, New_site), 
                                 by = c('Position' = 'Position')
)

dom_eff_tet_int_summary <- dom_eff_tet_int_new %>% ungroup() %>%
  group_by(New_site) %>%
  summarise(Obs_count = n())

data_deconv_tet <- left_join(x = dom_eff_tet_int_summary, 
                             y = expected_dom_eff_tet %>% mutate(New_site = Var1, Exp_count = Freq) %>%
                               select(New_site, Exp_count), 
                             by = c('New_site' = 'New_site')) %>%
  mutate(Rel_enrichment = round(Obs_count / Exp_count, 2))

p_deconv_tet <- data_deconv_tet %>% rowwise() %>%
  mutate(label_enrichment = str_c(toString(Rel_enrichment), '\n', 
                                  toString(Obs_count), '/', toString(Exp_count), 
                                  sep = ''), 
         New_site = factor(New_site, levels = c('Only tetramerization', 'NADPH binding'))) %>%
  ggplot(aes(x = New_site, y = Rel_enrichment)) +
  geom_bar(stat = 'identity') +
  geom_hline(yintercept = 1, linetype = 'dashed') +
  geom_label(aes(label = label_enrichment, x = New_site, y = Rel_enrichment + 1),
  ...
  ylab('Relative enrichment\n(neg. int. mutations)') +
  ylim(0, max_rel_enrichment + rel_enrichment_margin) +
  ggtitle('Tetramerization interface')
p_deconv_tet

## Buried residues ##

dom_eff_buried <- dom_eff_candidates_regions %>% filter(Site == 'Buried residues')

## Annotate positions from the dimerization interface that are also part of the active site 
test_positions_catalytic <- test_positions %>% 
  filter(Site == 'NADPH binding')

test_positions_dim <- test_positions %>% 
  filter(Site == 'Dimerization interface')

test_positions_tet <- test_positions %>% 
  filter(Site == 'Tetramerization interface')

test_positions_buried <- test_positions %>% filter(Site == 'Buried residues') %>%
  mutate(NADPH_binding = ifelse(Position %in% test_positions_catalytic$Position, 1, 0), 
         Dimerization = ifelse(Position %in% test_positions_dim$Position, 1, 0),
         Tetramerization = ifelse(Position %in% test_positions_tet$Position, 1, 0)
         ) %>% rowwise() %>%
  mutate(Only_buried = ifelse((NADPH_binding + Dimerization + Tetramerization) == 0, 1, 0))

total_missense_buried <- colSums(test_positions_buried %>% 
                                                 select(NADPH_binding, Dimerization, 
                                                        Tetramerization, Only_buried))*19

## Calculate expectations
data_tmp <- round(total_missense_buried*(total_candidates / total_missense), 2)
needed_rownames <- names(data_tmp)

expected_dom_eff_buried <- data.frame(data_tmp) %>%
  mutate(New_site = needed_rownames, Exp_count = data_tmp) %>%
  select(-data_tmp)
# NADPH_binding 8.22
# Dimerization 8.22
# Tetramerization 6.39
# Only_buried 10.04

## Get observed counts and add a column for expected counts
dom_eff_buried_new <- left_join(x = dom_eff_buried %>%
                                   select(WT_Residue, Position, Mutant_Residue, Selection_coefficient), 
                                 y = test_positions_buried %>%
                                   select(-Site), 
                                 by = c('Position' = 'Position')
)

data_tmp <- colSums(dom_eff_buried_new %>% select(NADPH_binding, Dimerization, 
                                                        Tetramerization, Only_buried))
new_sites <- names(data_tmp)

dom_eff_buried_summary <- data.frame(data_tmp) %>%
  mutate(New_site = new_sites, Obs_count = data_tmp) %>%
  select(-data_tmp)

data_deconv_buried <- left_join(x = dom_eff_buried_summary, 
                             y = expected_dom_eff_buried, 
                             by = c('New_site' = 'New_site')) %>%
  mutate(Rel_enrichment = round(Obs_count / Exp_count, 2))

p_deconv_buried <- data_deconv_buried %>% rowwise() %>%
  mutate(label_enrichment = str_c(toString(Rel_enrichment), '\n', 
                                  toString(Obs_count), '/', toString(Exp_count), 
                                  sep = ''), 
         New_site = case_when(
           New_site == 'Only_buried' ~ 'Only buried', 
           New_site == 'NADPH_binding' ~ 'NADPH binding', 
           TRUE ~ New_site
         )
  ) %>%
  mutate(New_site = factor(New_site, levels = c('Only buried', 'Dimerization',
                                                'Tetramerization', 'NADPH binding'))) %>%
  ggplot(aes(x = New_site, y = Rel_enrichment)) +
  geom_bar(stat = 'identity') +
  geom_hline(yintercept = 1, linetype = 'dashed') +
  geom_label(aes(label = label_enrichment, x = New_site, y = Rel_enrichment + 1),
  ...
  ylab('Relative enrichment\n(neg. int. mutations)') +
  ylim(0, max_rel_enrichment + rel_enrichment_margin) +
  ggtitle('Buried residues')
p_deconv_buried

#### NADPH binding sites ####
dom_eff_cat <- dom_eff_candidates_regions %>% filter(Site == 'NADPH binding')
...
test_positions_cat <- test_positions %>% filter(Site == 'NADPH binding') %>%
  mutate(
         Dimerization = ifelse(Position %in% test_positions_dim$Position, 1, 0),
         Tetramerization = ifelse(Position %in% test_positions_tet$Position, 1, 0)
  ) %>% rowwise() %>%
  mutate(Only_NADPH = ifelse((Dimerization + Tetramerization) == 0, 1, 0))

total_missense_cat <- colSums(test_positions_cat %>% 
                                   select(Dimerization, 
                                          Tetramerization, Only_NADPH))*19

## Calculate expectations
data_tmp <- round(total_missense_cat*(total_candidates / total_missense), 2)
needed_rownames <- names(data_tmp)

expected_dom_eff_cat <- data.frame(data_tmp) %>%
  mutate(New_site = needed_rownames, Exp_count = data_tmp) %>%
  select(-data_tmp)
# Dimerization 3.65
# Tetramerization 3.65
# Only_NADPH 5.48

## Get observed counts and add a column for expected counts
dom_eff_cat_new <- left_join(x = dom_eff_cat %>%
                                  select(WT_Residue, Position, Mutant_Residue, Selection_coefficient), 
                                y = test_positions_cat %>%
                                  select(-Site), 
                                by = c('Position' = 'Position')
)

data_tmp <- colSums(dom_eff_cat_new %>% select(Dimerization, 
                                                  Tetramerization, Only_NADPH))
new_sites <- names(data_tmp)

dom_eff_cat_summary <- data.frame(data_tmp) %>%
  mutate(New_site = new_sites, Obs_count = data_tmp) %>%
  select(-data_tmp)

data_deconv_cat <- left_join(x = dom_eff_cat_summary, 
                                y = expected_dom_eff_cat, 
                                by = c('New_site' = 'New_site')) %>%
  mutate(Rel_enrichment = round(Obs_count / Exp_count, 2))

p_deconv_cat <- data_deconv_cat %>% rowwise() %>%
  mutate(label_enrichment = str_c(toString(Rel_enrichment), '\n', 
                                  toString(Obs_count), '/', toString(Exp_count), 
                                  sep = ''), 
         New_site = case_when(
           New_site == 'Only_NADPH' ~ 'Only NADPH binding', 
           TRUE ~ New_site
         )
  ) %>%
  mutate(New_site = factor(New_site, levels = c('Only NADPH binding', 'Dimerization',
                                                'Tetramerization'))) %>%
  ggplot(aes(x = New_site, y = Rel_enrichment)) +
  geom_bar(stat = 'identity') +
  geom_hline(yintercept = 1, linetype = 'dashed') +
  ...
  ylab('Relative enrichment\n(neg. int. mutations)') +
  ylim(0, max_rel_enrichment + rel_enrichment_margin) +
  ggtitle('NADPH binding')
p_deconv_cat

## Plot four panels together
p_figS10 <- plot_grid(p_deconv_buried, p_deconv_dim, p_deconv_tet, p_deconv_cat,
                          nrow = 1, labels = c('A', 'B', 'C', 'D'),
                          label_size = panel_label_size, label_fontface = 'bold')

ggsave(p_figS10, device = cairo_pdf, width = 35, height = 12, units = 'cm', dpi = 300, 
       filename = 'Figures/Supp_figures/FigS10.pdf')
...
mut_counts <- all_dms_foldx_DN_long_unique %>% ungroup() %>% 
  ...
  summarise(count = n())
## When using standard deviations
# 63 negative interference mutations
# 1076 other mutations

## When using standard error of the mean
# 318 negative interference mutations
# 821 other mutations

interfering_count <- mut_counts$count[[1]]
other_count <- mut_counts$count[2]

## Calculate median ddGs
median_ddg_vals <- all_dms_foldx_DN_long_unique %>% ungroup() %>%
  mutate(ddG_type = factor(ddG_type,
                           levels = c('Folding', 'Dimerization', 'Tetramerization', 
                                      'NADPH binding', 'DHF binding')), 
         DN_bool = case_when(
           DN_bool == 'Neg. interference' ~ str_c('Neg. interference\n(n = ', toString(interfering_count), ')', sep = ''),
           DN_bool == 'Other' ~ str_c('Other (n = ', toString(other_count), ')', sep = '')
         )) %>%
  group_by(ddG_type, DN_bool) %>%
  summarise(median_ddG = round(median(ddG_value), 2))
...
p_fig3B <- all_dms_foldx_DN_long_unique %>% ungroup() %>%
  ...
  geom_signif(data = as.data.frame(comps) %>% ungroup() %>%
                ...
                mutate(group1 = case_when(
                  group1 == 'Neg. interference' ~ str_c('Neg. interference\n(n = ', toString(interfering_count), ')', sep = ''),
                  group1 == 'Other' ~ str_c('Other (n = ', toString(other_count), ')', sep = '')
                ), 
                group2 = case_when(
                  group2 == 'Neg. interference' ~ str_c('Neg. interference\n(n = ', toString(interfering_count), ')', sep = ''),
                  group2 == 'Other' ~ str_c('Other (n = ', toString(other_count), ')', sep = '')
                )),
              ...
              tip_length = 1) +
  geom_label(data = median_ddg_vals, inherit.aes = T, 
            aes(label = median_ddG, x = DN_bool), y = -2.75)
p_fig3B
...
ggsave(p_fig3, device = cairo_pdf,
       width = 26, height = 20,
       dpi = 300, units = 'cm',
       filename = 'Figures/Main_figures/Fig3_noAnnotations_newPanel3B.pdf'
       # filename = 'Figures/Main_figures/Fig3_noAnnotations_sem_Interference.pdf'
...
#### Fig. S9: Comparison of effects to other mutations from the same region ####
...
p_figS9
ggsave(p_figS9, device = cairo_pdf, width = 20, height = 24, units = 'cm', dpi = 300,  
