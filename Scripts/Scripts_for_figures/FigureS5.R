...
p_figS5 <- all_data_processed_expression %>%
  mutate(Strain = factor(Strain,
                         levels = c('WT (neg ctl)', 'WT-sfGFP', 'E2R-sfGFP',
                                    'N15D-sfGFP', 'F18W-sfGFP', 'S20M-sfGFP'))) %>%
  ...
  geom_text(data = tukey_annotation %>% 
              mutate(Strain = factor(Strain,
                                     levels = c('WT (neg ctl)', 'WT-sfGFP', 'E2R-sfGFP',
                                                'N15D-sfGFP', 'F18W-sfGFP', 'S20M-sfGFP'))), size = 5, inherit.aes = F,
            aes(label = groups, x = as.factor(Strain), y = y))
p_figS5
ggsave(p_figS5, device = cairo_pdf, width = 16, height = 10, dpi = 300, units = 'cm', 
